...
public class Camera extends Subsystem {
    ...
    private final TargetReport target = new TargetReport();
    private final AxisCamera camera;
    private final CriteriaCollection cc;
    private final int verticalTargets[] = new int[RobotConstants.MAX_PARTICLES];
    private final int horizontalTargets[] = new int[RobotConstants.MAX_PARTICLES];
    private int verticalTargetCount, horizontalTargetCount;

    public Camera() {
        camera = AxisCamera.getInstance();
        cc = new CriteriaCollection();
        cc.addCriteria(MeasurementType.IMAQ_MT_AREA, RobotConstants.AREA_MINIMUM, 65535, false);
    }
    ...
    double computeDistance(BinaryImage image, ParticleAnalysisReport report, int particleNumber) throws NIVisionException {
        ...
        return RobotConstants.Y_IMAGE_RES * targetHeight / (height * 12 * 2 * Math.tan(RobotConstants.VIEW_ANGLE * Math.PI / (180 * 2)));
    }
